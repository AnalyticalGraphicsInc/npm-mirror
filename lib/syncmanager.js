...
SyncManager.prototype = {
  ...
  packageVersionSearch: function(package, version, callback) {
    ...
    async.waterfall([
      ...
      function(done) {
        var packageUrl = Package.url(this.master, package);
        Download.inst.download(packageUrl, done);
      }.bind(this),

      // Parse package version data.
      function(packageDataJson, done) {
        var packageData = JSON.parse(packageDataJson);
        var packageVersion = packageData.versions[version];
    ...
    async.waterfall([
      ...
      function(done) {
        ...
        for (var package in this.packageToVersions) {
          dirs.push(Package.localDir(this.tempdir, package));
          var versions = this.packageToVersions[package];
          for (var version in versions) {
            dirs.push(Package.localDir(this.tempdir, package, version));
          }
        }
  ...
  downloadPackageRootObjects: function(callback) {
    ...
    async.parallel(packages.map(function(package) {
      var packageRootUrl = Package.url(this.master, package);
  ...
  downloadPackageRootObject: function(package, packageRootUrl, callback) {
    ...
    async.waterfall([
      ...
      function(packageRootData, done) {
        ...
        var dest = path.resolve(Package.localDir(this.tempdir, package), 'index.json');
        fs.writeFile(dest, JSON.stringify(copy, null, 4), done);
      }.bind(this)
    ], function(err) {
      return callback && callback(err);
    });
  ...
  downloadPackageVersion: function(package, version, callback) {
    async.waterfall([
      ...
      function(done) {
        var dest = path.resolve(Package.localDir(this.tempdir, package, version), 'index.json');
      ...
      function(packageDataJson, done) {
        var packageData = JSON.parse(packageDataJson);
        var packageVersion = packageData.versions[version];
        var copy = JSON.parse(JSON.stringify(packageVersion));
        if ('dist' in copy) {
          copy.dist.tarball =
        ...
        var dest = path.resolve(Package.localDir(this.tempdir, package, version), 'index.json');
        fs.writeFile(dest, JSON.stringify(copy, null, 4), function(err) {
      ...
      function(packageVersion, done) {
        ...
        var dest = Package.localTarball(this.tempdir, package, version);
        mkdirp.sync(path.dirname(dest), {fs:fs});
  ...
  verifyPackageVersion: function(package, version, callback) {
    var manifest = path.resolve(Package.localDir(this.tempdir, package, version), 'index.json');
    var packageVersion = JSON.parse(fs.readFileSync(manifest, 'utf8'));
    ...
    var tarball = Package.localTarball(this.tempdir, package, version);
