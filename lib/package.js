...
var Package = {
   ...
   */
  url: function(hostname, package) {
    var resource = package.replace('/', '%2F');
    return url.resolve(hostname, resource);
  },

  localDir : function(rootdir, package, version) {
    if (version) {
      return path.resolve(rootdir, package, version);
    } else {
      return path.resolve(rootdir, package);
    }
  },
  ...
  tarballUrl: function(hostname, package, version) {
    var resource = package.replace('/', '%2F');
    var tarball = resource + '-' + version + '.tgz';
    return url.resolve(hostname, [resource, version, tarball].join('/'));
  },

  localTarball: function(rootdir, package, version) {
    var tarball = package + '-' + version + '.tgz';
    return path.resolve(rootdir, [package, version, tarball].join('/'));
  },
